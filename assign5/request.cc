...
void HTTPRequest::addRequestEntity(const string& clientIPAddress) {
  requestHeader.addHeader("x-forwarded-proto", "http");
  if (requestHeader.containsName("x-forwarded-for")) {
    string v = requestHeader.getValueAsString("x-forwarded-for") + ", ";
      cout<<"now the value is "<<v<<endl;
    requestHeader.addHeader("x-forwarded-for", v + clientIPAddress);
      cout<<"Added "<<v + clientIPAddress<<endl;
