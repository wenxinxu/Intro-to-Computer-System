...
MapReduceServer::MapReduceServer(int argc, char *argv[]) throw (MapReduceServerException) 
  : user(getUser()), host(getHost()), cwd(getCurrentWorkingDirectory()),
    ...
    serverIsRunning(false), pool(kNumWorkerThreads) {
...
bool MapReduceServer::surfaceNextFilePattern(std::string& pattern) throw() {
 /* lock_guard<std::mutex> lg(surfacelock);
  if(unprocessed.empty()) {
      if(!inflight.empty()) {
          cv.wait(surfacelock, [this]{return inflight.empty();});
      }
...
void MapReduceServer::markFilePatternAsProcessed(const std::string& clientIPAddress, 
  ...
  std::lock_guard<mutex> lg(markProcessed);
...
void MapReduceServer::rescheduleFilePattern(const std::string& clientIPAddress, 
                                            const std::string& pattern) throw() {
  std::lock_guard<mutex> lg(reschedulePattern);
  inflight.erase(pattern);
 // cv.notify_all();
...
void MapReduceServer::spawnMappers() throw() {
  ...
  for(size_t i = 0; i < kNumWorkerThreads; i++) {
    ...
    string node = mapperNodes[0];
    string command = buildMapperCommand(node, mapperExecutable, intermediatePath, numMappers*numReducers);
 ...
 *
 */
string MapReduceServer::buildReducerCommand(const string& remoteHost, const string& executable,
 ...
 * Method: spawnReducer()
 * ----------------
 * similar functionality as spawnmapper()
 */
void MapReduceServer::spawnReducer() throw(){
  unprocessed.clear();
  vector<string> reducerNodes(nodes);
  for(size_t i = 0; i < numMappers*numReducers; i++) {
    string file = intermediatePath + "/*." + numberToString(i) + ".mapped";
    unprocessed.push_back(file);
  }
  for(size_t i = 0; i< kNumWorkerThreads; i++) {
    random_shuffle(reducerNodes.begin(), reducerNodes.end());
    string node = reducerNodes[0];
    string command = buildReducerCommand(node, reducerExecutable, outputPath);
    pool.schedule([this,command,node](){
        spawnWorker(node,command);
    });
  }
