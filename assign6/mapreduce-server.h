...
class MapReduceServer {
 ...
 private:
  ...
  void spawnReducer() throw();
  std::string buildReducerCommand(const std::string& remoteHost, const std::string& executable,
                        const std::string& outputPath) throw();//, size_t splitValue) throw();
  ...
  ThreadPool pool;
  std::mutex spawnlock;
  std::mutex markProcessed;
  std::mutex reschedulePattern;
